...
function drawWire(wire) {
    ...
    points.forEach(point => path.lineBy(point));

    return path;
}

let wirePathOne = drawWire(wireOne);
let wirePathTwo = drawWire(wireTwo);

const distances = wirePathOne.getIntersections(wirePathTwo).slice(1).map(function(intersection) {    
    return wirePathOne.getOffsetOf(intersection.point) + wirePathTwo.getOffsetOf(intersection.point);
});
